  ...
  <div>
    ...
    <div class="hpHeader pt-5 pb-3">
      <div class="w-100 d-flex justify-content-center text-center">
        <div class="title-block text-white px-5" v-scrollanimation>
          <h1 class="h2 title">KeyBoard Shop</h1>
    ...
    <section class="recommend py-5">
      <div class="container">
        ...
        <div class="row">
          <div class="col-md-4 mb-4 box-shadow hpkeyboardCard" v-for="item in randomProduct" :key="item.id">
...
export default {
  ...
  methods: {
    ...
    emailSubmit () {
      const vm = this
      vm.emailResult = ''
      vm.$bus.$emit('successMessage', '訂閱成功!', 'success')
    }
  },
  computed: {
    randomProduct () {
      const vm = this
      if (vm.singleProduct.length === 0) {
        return
      }
      ...
      let n = 0
      const temp = []
      n = randomList(vm.singleProduct.length)
      for (let i = 0; i < 3; i += 1) {
        n = randomList(vm.singleProduct.length)
        temp.push(n)
      }
      ...
      recommendObjList = [
        vm.singleProduct[result[0]],
        vm.singleProduct[result[1]],
        vm.singleProduct[result[2]]
      ]
